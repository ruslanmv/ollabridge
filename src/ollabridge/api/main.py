...
def create_app() -> FastAPI:
    ...
    # CORS: always enabled so browser clients (3D Avatar, etc.) can reach the API.
    # Use configured origins if provided, otherwise allow all origins.
    ...
    if (settings.AUTH_MODE or "").lower().strip() == "pairing":
        ...
        mgr = PairingManager()
        # If the CLI generated a pairing code before uvicorn started,
        # inject it so clients can exchange it via POST /pair.
