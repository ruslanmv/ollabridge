...
It also exposes an additive, authenticated HomePilot image-generation bridge:

- GET /v1/media/homepilot/capability
- POST /v1/media/homepilot/generate

The browser authenticates only to OllaBridge. HomePilot credentials remain
server-side and generated image bytes are returned directly to the caller.
...
def _hp_api_key() -> str:
    cfg = _hp_config()
    return str(cfg.get("homepilot_api_key") or settings.HOMEPILOT_API_KEY or "").strip()
...
async def homepilot_image_capability() -> dict[str, Any]:
    """Report whether the configured HomePilot can accept Imagine requests.

    This is intentionally a live, read-only probe. It never starts generation.
    Older or disabled HomePilot installations degrade to ``available: false`` so
    clients can keep another generator selected without breaking chat.
    """
    if not _hp_enabled():
        return {
            "ok": True,
...
def _generated_image_url(base: str, value: str) -> str:
    """Resolve an image location returned by HomePilot.

    The browser never supplies ``value``. It comes only from the authenticated
    HomePilot response, which keeps this endpoint from becoming an arbitrary
    user-controlled fetch proxy.
    """
...
async def homepilot_generate_image(
    ...
    """Generate one image through HomePilot Imagine and return the image bytes.

    The endpoint intentionally returns bytes rather than a HomePilot/ComfyUI URL:
    the paired browser never needs HomePilot credentials and never learns an
    internal media host. OllaBridge remains the only authenticated boundary.
    """
    if not _hp_enabled():
