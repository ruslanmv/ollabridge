...
class CloudBridgeManager:
    """Manage the lifecycle of the WebSocket bridge to OllaBridge Cloud."""
    ...
    async def _fetch_cloud_manifest(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            log.error(
                "Unable to build approved cloud model manifest (publishing nothing — failing closed): %s",
                exc,
            )
            return []
        ...
        while not self._stop_event.is_set():
            try:
                ...
                async with ws_connect(ws_url, **connect_options) as ws:
                    ...
                    log.info("Registered %d models with cloud: %s", len(models), models[:8])
                    refresh_task = asyncio.create_task(self._model_refresh_loop(ws))
                    try:
                        async for raw in ws:
                ...
                if _is_auth_rejection(exc):
                    log.error(
                        "Cloud rejected the device token — this device was unlinked or its access revoked in OllaBridge Cloud. Stopping reconnect; re-pair to reconnect."
                    )
                    self.status.state = BridgeState.ERROR
                    self.status.last_error = (
                        "Device unlinked from OllaBridge Cloud (access revoked). Re-pair from the dashboard to reconnect."
                    )
                    self._ws = None
                    break
